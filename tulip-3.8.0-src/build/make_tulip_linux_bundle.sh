...
BUNDLE_DIR=$OUTPUT_DIR/Tulip-3.8.0
...
cp -Rp /usr/local/bin $BUNDLE_DIR
echo /usr/local/bin copied in $BUNDLE_DIR
cp -Rp /usr/local/lib $BUNDLE_DIR
echo /usr/local/lib copied in $BUNDLE_DIR
cp -Rp /usr/local/share $BUNDLE_DIR
echo /usr/local/share copied in $BUNDLE_DIR
...
# first Qt libs
cp /usr/lib/x86_64-linux-gnu/libQt*.so.4 $BUNDLE_LIB_DIR
echo '/usr/lib/x86_64-linux-gnu/libQt*.so.4' copied in $BUNDLE_LIB_DIR
cp /usr/lib/x86_64-linux-gnu/libphonon.so.4 $BUNDLE_LIB_DIR
echo /usr/lib/x86_64-linux-gnu/libphonon.so.4 copied in $BUNDLE_LIB_DIR
...
# copy Qt assistant
cp -p /usr/bin/assistant $BUNDLE_BIN_DIR
echo /usr/bin/assistant copied in $BUNDLE_BIN_DIR
...
cp -Rp /usr/lib/x86_64-linux-gnu/qt4/plugins/sqldrivers $BUNDLE_BIN_DIR
cp -Rp /usr/lib/x86_64-linux-gnu/qt4/plugins/imageformats $BUNDLE_BIN_DIR
echo /usr/lib/x86_64-linux-gnu/qt4/plugins/sqldrivers copied in $BUNDLE_BIN_DIR/plugins

# tweak OS/ABI header part of libraries (System V instead of Linux)
for LIB in $(find $OUTPUT_DIR/Tulip-3.8.0 -name '*.so*')
do
...
cd $OUTPUT_DIR/Tulip-3.8.0
# add toplevel link
ln -s ./bin/tulip tulip
cd ..
PROC_TYPE=$(uname -m)
tar zcvf $OUTPUT_DIR/Tulip-3.8.0-${PROC_TYPE}-bundle.tar.gz Tulip-3.8.0
echo linux bundle $OUTPUT_DIR/Tulip-3.8.0-${PROC_TYPE}-bundle.tar.gz built
